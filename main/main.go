...
import (
	...
	"matching-engine/engine_monitor"
)
...
func main () {
	...
	go func() {
		em:= engine_monitor.Monitor_Server{}
		em.StartMointor(&book)
	}()
...
func createConsumer() *cluster.Consumer {
	...
	// create the consumer
	consumer, err := cluster.NewConsumer([]string{"127.0.0.1:19092"}, "myconsumer", []string{"orders"}, config)
	if err!=nil {
		fmt.Println(err)
...
func createProducer() sarama.AsyncProducer {
	...
	producer, err := sarama.NewAsyncProducer([]string{"127.0.0.1:19092"}, config)
	if err!=nil {
